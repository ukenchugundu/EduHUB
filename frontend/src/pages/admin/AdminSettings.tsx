...
const readStoredSettings = (): AdminAppSettings => {
  try {
    ...
    return {
      enforceOtpLogin: Boolean(
        parsed.enforceOtpLogin ?? defaultSettings.enforceOtpLogin,
      ),
      allowPasswordResetEmail: Boolean(
        parsed.allowPasswordResetEmail ??
        defaultSettings.allowPasswordResetEmail,
      ),
      autoCloseQuizAttempts: Boolean(
        parsed.autoCloseQuizAttempts ?? defaultSettings.autoCloseQuizAttempts,
      ),
      requireFacultyScorePublish: Boolean(
        parsed.requireFacultyScorePublish ??
        defaultSettings.requireFacultyScorePublish,
      ),
      notifyOnNewSubmission: Boolean(
        parsed.notifyOnNewSubmission ?? defaultSettings.notifyOnNewSubmission,
      ),
      maintenanceMode: Boolean(
        parsed.maintenanceMode ?? defaultSettings.maintenanceMode,
      ),
      supportEmail: String(
        parsed.supportEmail ?? defaultSettings.supportEmail,
      ).trim(),
      defaultQuizDurationMins: Number(
        parsed.defaultQuizDurationMins ??
          defaultSettings.defaultQuizDurationMins,
      ),
    };
  } catch {
    return defaultSettings;
...
const AdminSettings = () => {
  ...
  const updateSetting = <K extends keyof AdminAppSettings>(
    key: K,
  ...
  const handleSaveSettings = () => {
    ...
    if (
      settings.defaultQuizDurationMins < 5 ||
      settings.defaultQuizDurationMins > 240
    ) {
      setSaveMessage("");
    ...
    try {
      ...
      setSaveError(
        refreshError instanceof Error
          ...
          : "Failed to refresh website data.",
      );
    } finally {
      ...
      <div className="space-y-6">
        ...
        {isError ? (
          <p className="text-sm text-destructive">
            {error?.message ?? "Failed to load settings snapshot."}
          </p>
        ) : null}
        {saveError ? (
            ...
            <div className="space-y-3 text-sm">
              <label className="flex items-start gap-3">
                <input
                  ...
                  onChange={(event) =>
                    updateSetting("enforceOtpLogin", event.target.checked)
                  }
                />
                <span>
                  ...
                  <span className="block text-muted-foreground">
                    Require OTP step for all roles.
                  </span>
                </span>
              </label>
              <label className="flex items-start gap-3">
                ...
                <span>
                  <span className="font-medium text-foreground">
                    Allow password-reset email
                  </span>
                  <span className="block text-muted-foreground">
                    Keep reset-link flow active.
                  </span>
                </span>
              </label>
              <label className="flex items-start gap-3">
                ...
                <span>
                  ...
                  <span className="block text-muted-foreground">
                    Mark platform for maintenance window.
                  </span>
                </span>
              </label>
            </div>
            ...
            <div className="space-y-3 text-sm">
              <label className="flex items-start gap-3">
                <input
                  ...
                  onChange={(event) =>
                    updateSetting("autoCloseQuizAttempts", event.target.checked)
                  }
                />
                <span>
                  <span className="font-medium text-foreground">
                    Auto-close quiz attempts
                  </span>
                  <span className="block text-muted-foreground">
                    Close attempts after submit/time-over.
                  </span>
                </span>
              </label>
              <label className="flex items-start gap-3">
                ...
                <span>
                  <span className="font-medium text-foreground">
                    Manual faculty score publish
                  </span>
                  <span className="block text-muted-foreground">
                    Publish student-visible scores after review.
                  </span>
                </span>
              </label>
            ...
            <div className="space-y-3 text-sm">
              <label className="flex items-start gap-3">
                <input
                  ...
                  onChange={(event) =>
                    updateSetting("notifyOnNewSubmission", event.target.checked)
                  }
                />
                <span>
                  <span className="font-medium text-foreground">
                  ...
                  <span className="block text-muted-foreground">
                    Alert faculty for new attempts/submissions.
                  </span>
                </span>
              </label>
              <div className="space-y-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-sm">
            <div className="rounded-xl bg-secondary/40 p-3">
              ...
              <p className="text-xl font-bold text-foreground">
                {isLoading ? "..." : systemStats.quizzesCount}
              </p>
            </div>
            <div className="rounded-xl bg-secondary/40 p-3">
              <p className="text-xs text-muted-foreground">Assignments</p>
              <p className="text-xl font-bold text-foreground">
                {isLoading ? "..." : systemStats.assignmentsCount}
              </p>
            </div>
            <div className="rounded-xl bg-secondary/40 p-3">
              <p className="text-xs text-muted-foreground">Notes</p>
              <p className="text-xl font-bold text-foreground">
                {isLoading ? "..." : systemStats.notesCount}
              </p>
            </div>
            <div className="rounded-xl bg-secondary/40 p-3">
              <p className="text-xs text-muted-foreground">
