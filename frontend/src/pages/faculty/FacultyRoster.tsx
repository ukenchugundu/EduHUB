...
const FacultyRoster = () => {
  ...
  const classSummaries = useMemo<ClassPerformanceSummary[]>(() => {
    ...
    for (const quiz of quizRows) {
      ...
      const studentsMap =
        classMap.get(className) ?? new Map<string, StudentMapValue>();
    ...
    const summary: ClassPerformanceSummary[] = Array.from(
      classMap.entries(),
    ).map(([className, studentsMap]) => {
      const students = Array.from(
        studentsMap.values(),
      ).map<StudentPerformanceSummary>((student) => {
        ...
        const lastQuizTime = student.quizRows
          .map((quiz) =>
            quiz.submitted_at ? new Date(quiz.submitted_at).getTime() : 0,
          )
        ...
        return {
          ...
          quizzesReviewed: student.quizRows.filter(
            (quiz) => quiz.faculty_score !== null,
          ).length,
          ...
          lastSubmissionAt:
            lastSubmissionTime > 0
              ? new Date(lastSubmissionTime).toISOString()
              : null,
        };
      });
      ...
      return {
        ...
        quizAttempts: students.reduce(
          (sum, student) => sum + student.quizzesAttempted,
          0,
        ),
  ...
  const filteredStudents = useMemo(() => {
    if (!activeClassSummary) return [];

    let students = activeClassSummary.students;

    if (activeTab === "literacy") {
      // Filter students with literacy >= 80%
      students = students.filter((s) => {
        const avg = calculateAverage([
          s.quizAveragePercent,
          s.assignmentAveragePercent,
        ]);
        return avg !== null && avg >= 80;
      });
    } else if (activeTab === "attendance") {
      // Filter students with attendance >= 90%
      students = students.filter((s) => {
        const avg = calculateAverage([
          s.quizAveragePercent,
          s.assignmentAveragePercent,
        ]);
        return avg !== null && avg >= 85;
      });
    } else if (activeTab === "high") {
      // High performers: quiz + assignment average >= 75%
      students = students.filter((s) => {
        const avg = calculateAverage([
          s.quizAveragePercent,
          s.assignmentAveragePercent,
  ...
  const selectedStudentSummary = useMemo(() => {
    ...
    return (
      activeClassSummary.students.find(
  ...
  const studentQuizRows = useMemo(() => {
    ...
    return quizRows
      .filter(
        (quiz) => normalizeClassName(quiz.cls) === activeClassSummary.className,
      )
  ...
  const studentAssignmentRows = useMemo(() => {
    ...
    return assignmentRows
      .filter(
        (assignment) =>
          normalizeClassName(assignment.cls) === activeClassSummary.className,
      )
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Select a class, review student-wise performance, then drill into
        ...
        {!isLoading && !isError && classSummaries.length === 0 ? (
          <div className="glass-card rounded-2xl p-5 text-sm text-muted-foreground">
            No class-level performance data yet. Once students submit
            quizzes/assignments, this dashboard will populate.
          </div>
        ) : null}
                  ...
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div>
                      <h2 className="font-heading text-xl font-bold text-foreground">
                        {activeClassSummary.className} - Students Performance
                      </h2>
                      <p className="text-sm text-muted-foreground">
                        Click a student to open complete performance and
                        submission reports.
                      </p>
                    </div>
                    {selectedStudentId ? (
                  ...
                  {!selectedStudentId && (
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        ...
                        Total Students ({activeClassSummary.students.length})
                      </button>
                      <button
                          ...
                          {filteredStudents.map((student) => {
                            ...
                            return (
                              <tr
                                ...
                                onClick={() =>
                                  navigate(
                                    `/faculty/performance/${student.studentId}`,
                                  )
                                }
                              >
                                ...
                                <td className="p-4 text-center text-destructive font-medium">
                                  {pending}
                                </td>
                                <td className="p-4 text-center">
                                  <Button
                  ...
                  <div className="space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                      ...
                      <div className="glass-card rounded-2xl p-4">
                        ...
                        <p className="text-2xl font-heading font-bold text-foreground mt-1">
                          {selectedStudentSummary.quizzesReviewed}/
                          {selectedStudentSummary.quizzesAttempted}
                        </p>
                      </div>
                      <div className="glass-card rounded-2xl p-4">
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="text-2xl font-heading font-bold text-foreground mt-1">
                          {selectedStudentSummary.assignmentsReviewed}/
                    ...
                    <div className="glass-card rounded-2xl p-5">
                      <h3 className="font-heading font-semibold text-foreground mb-3 flex items-center gap-2">
                        <BarChart3 className="w-4 h-4 text-accent" /> Quiz
                        Performance
                      </h3>
                      {studentQuizRows.length === 0 ? (
                        <p className="text-sm text-muted-foreground">
                          No quiz attempts available for this student.
                        </p>
                      ) : (
                    ...
                    <div className="glass-card rounded-2xl p-5">
                      <h3 className="font-heading font-semibold text-foreground mb-3 flex items-center gap-2">
                        <FileText className="w-4 h-4 text-accent" /> Assignment
                        Performance
                      </h3>
                      {studentAssignmentRows.length === 0 ? (
                        <p className="text-sm text-muted-foreground">
                          No assignment submissions available for this student.
                        </p>
                      ) : (
                        <div className="space-y-2">
                            ...
                            >
                              ...
                              <div className="mt-1 text-xs text-muted-foreground flex flex-wrap gap-4">
                                ...
                                <span className="flex items-center gap-1">
                                  <CheckCircle2 className="w-3.5 h-3.5 text-accent" />{" "}
                                  Reviewed:{" "}
                                  {formatDateTime(assignment.reviewed_at)}
                                </span>
                              </div>
                            </div>
                          ))}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                      <div className="glass-card rounded-2xl p-5">
                        <h3 className="font-heading font-semibold text-foreground mb-3 flex items-center gap-2">
                          <BookOpen className="w-4 h-4 text-accent" /> Exam
                          Results
                        </h3>
                        {examRows.length === 0 ? (
                          <p className="text-sm text-muted-foreground">
                            No exam-tagged quiz records found. Exam reports will
                            appear when quiz titles include terms like
                            Exam/Mid/Final.
                          </p>
                        ) : (
                          <div className="space-y-2 text-sm">
                            {examRows.map((exam) => (
                              ...
                              >
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                  Score: {exam.faculty_score ?? "-"}/
                                  {exam.total_questions} | Reviewed:{" "}
                                  {formatDateTime(exam.reviewed_at)}
                                </p>
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="p-3 rounded-xl bg-secondary/50 flex items-center justify-between">
                            ...
                            <span className="font-semibold text-destructive">
                              {studentQuizRows.filter(
                                (quiz) => quiz.faculty_score === null,
                              ).length +
                                studentAssignmentRows.filter(
                                  (assignment) =>
                                    assignment.faculty_score === null,
                                ).length}
                            </span>
                          </div>
                          <div className="p-3 rounded-xl bg-secondary/50 flex items-center justify-between">
                            ...
                            <span className="font-semibold text-foreground text-xs">
                              {formatDateTime(
                                selectedStudentSummary.lastSubmissionAt,
                              )}
                            </span>
                          </div>
                        </div>
