...
const fetchFacultyResults = async (
  signal?: AbortSignal,
  ...
  try {
    ...
    if (!response.ok) {
      const message = await readApiErrorMessage(
        response,
        "Failed to fetch faculty results.",
      );
...
const FacultyResults = () => {
  ...
  >({
    ...
    refetchInterval: 5000, // Poll every 5 seconds for new submissions
    refetchIntervalInBackground: true,
    staleTime: 0,
    retry: 1,
  });

  // Track previous row count for notifications
  const previousCountRef = useRef<number>(0);
  
  useEffect(() => {
    if (data && data.length > previousCountRef.current && previousCountRef.current > 0) {
      const newSubmissions = data.length - previousCountRef.current;
      toast.info(`📝 ${newSubmissions} new submission${newSubmissions > 1 ? 's' : ''} received!`, {
        duration: 4000,
      });
    }
  ...
  const uploadMutation = useMutation({
    ...
    onError: (err) => {
      const message =
        err instanceof Error ? err.message : "Failed to upload score.";
      toast.error(message);
    },
  });
  ...
  const rows = useMemo(
    () =>
      (data ?? [])
        .filter((row) => {
          if (quizId && String(row.quiz_id) !== quizId) return false;
          if (filterStudentId && row.student_id !== filterStudentId)
            return false;
          return true;
        })
        .slice()
        .sort((a, b) => b.attempt_id - a.attempt_id),
    [data, quizId, filterStudentId],
  );
  const pendingCount = rows.filter((row) => row.faculty_score === null).length;
  const publishedCount = rows.filter(
    (row) => row.faculty_score !== null,
  ).length;
      ...
      <div className="space-y-6">
        ...
        {!isLoading && !isError && rows.length === 0 && (
          <p className="text-sm text-muted-foreground">
            No submitted quiz attempts yet. Student submissions will appear
            here.
          </p>
        )}
