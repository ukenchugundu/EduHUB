      ...
      <div className="space-y-6">
        ...
        <div className="glass-card rounded-2xl p-4 border-primary/20 flex items-center gap-3 bg-primary/5">
          ...
          <div className="text-sm text-foreground">
            <p className="font-medium mb-1">🔒 Secure Quiz Environment</p>
            <p>
              Quizzes run in <strong>fullscreen lockdown mode</strong>.
              Navigation is disabled until submission. Tab switching,
              right-click, and keyboard shortcuts are blocked for academic
              integrity.
            </p>
          </div>
        </div>
        ...
        {isError && (
          <p className="text-destructive text-sm">{fetchErrorMessage}</p>
        )}
        {!isLoading && !isError && quizList.length === 0 && (
          <p className="text-sm text-muted-foreground">
            No quizzes available yet. New quizzes from faculty will appear here
            automatically.
          </p>
        )}
        ...
        <div className="space-y-3">
          {quizList.map((q, i) => {
            const questionCount = Array.isArray(q.questions)
              ? q.questions.length
              : 0;
            return (
              <motion.div
              ...
              >
                ...
                {getStatusPill(q.status, () =>
                  navigate(`/student/quizzes/${q.quiz_id}`),
                )}
              </motion.div>
            );
          })}
