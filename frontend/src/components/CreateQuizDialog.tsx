...
type QuizStatus = "Published" | "Unpublished";
...
const clampDurationMinutes = (value: number): number =>
  Math.max(
    ...
    Math.min(MAX_QUIZ_DURATION_MINUTES, Math.round(value)),
  );
  ...
  if (clockMatch) {
    ...
    if (clockMatch[3]) {
      return clampDurationMinutes(
        first * 60 + Math.round((second * 60 + third) / 60),
      );
    }
...
const normalizeEditableQuestions = (
  ...
  return questions.map((rawQuestion) => {
    ...
    const correctFromQuestionColumn =
      typeof rawQuestion.correct_answer_text === "string"
        ? rawQuestion.correct_answer_text
        : "";
    ...
    const firstNonEmptyOption =
      rawOptions.find((optionText) => optionText.trim().length > 0) ?? "";
...
const CreateQuizDialog = ({
  ...
  const [requestedStatus, setRequestedStatus] = useState<QuizStatus | null>(
    null,
  );
  ...
  const mutation = useMutation({
    mutationFn: async (params: {
      ...
      try {
        ...
        if (isEditMode) {
          try {
            await fetch(`${API_BASE}/api/reset-quiz-attempts/${quizId}`, {
              method: "POST",
              signal: controller.signal,
            });
          } catch (resetError) {
            console.warn("Could not reset quiz attempts:", resetError);
          }
        }

        // For new quizzes, also set the status if provided
        if (!isEditMode && params.statusAfterSave) {
          const statusResponse = await fetch(
            `${QUIZZES_API_URL}/${updatedQuiz.quiz_id}/status`,
            {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
    ...
    onSuccess: (_data, variables) => {
      ...
      if (mode === "edit") {
        if (variables.statusAfterSave === "Published") {
          toast.success(
            "Quiz updated and published successfully! Students can attempt again.",
          );
        } else if (variables.statusAfterSave === "Unpublished") {
          toast.success(
            "Quiz updated and saved as unpublished. Students can attempt again.",
          );
        } else {
          toast.success(
            "Quiz updated successfully! Students can attempt again.",
          );
        }
      } else {
    ...
    return (
      ...
      questions.every((q) => {
        ...
        return (
          ...
          validOptions.includes(q.correctAnswer.trim())
        );
      })
    );
  };
  ...
  const saveDraftText =
    mutation.isPending && requestedStatus === "Draft"
      ? "Saving..."
      : "Save as Draft";
        ...
        <AnimatePresence mode="wait">
                    ...
                    {isNewClass && (
                      <div className="mt-2">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Creating a new class will add it to the list for
                          future quizzes.
                        </p>
                      </div>
                    )}
                ...
                <div className="mt-2 grid gap-3">
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto]">
                    <Input
                      ...
                      onChange={(e) =>
                        handleDurationMinutesChange(e.target.value)
                      }
                      required
                    />
                  ...
                  <p className="text-xs text-muted-foreground">
                    Students will get {formatTimerPreview(durationMinutes)} to
                    complete this quiz.
                  </p>
                </div>
              </div>
          ...
          {step === 2 && (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                Quiz questions are auto-shuffled per student when they start the
                attempt.
              </p>
              <p className="text-xs text-muted-foreground">
              ...
              <div className="flex flex-wrap justify-between gap-2">
                ...
                {isEditMode ? (
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {mutation.isPending && requestedStatus === "Unpublished"
                        ? "Saving..."
                        : "Save as Unpublished"}
                    </Button>
                    <Button
