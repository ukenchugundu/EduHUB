...
export const isAcceptedProfileImageFile = (file: File): boolean => {
  // Check extension first since browsers may not recognize all MIME types
  // (especially for newer formats like HEIC, AVIF, WebP on some systems)
...
export const getProfileIdentifierLabel = (
  session: StoredAuthSession | null,
): string => {
...
export const getProfileIdentifierValue = (
  session: StoredAuthSession | null,
): string => {
  if (!session) {
    return "-";
